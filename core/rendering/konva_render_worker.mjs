...
function buildContent(sentence, elementType) {
  const srcText = sentence.original_text || sentence.text || sentence.source_text || ''
  const tgtText =
...
function resolveFontsDir() {
  const envDir = process.env.LINGUA_FONT_DIR
  const candidates = [
    envDir,
  ...
  // Default to persistent directory even if it does not exist yet.
...
async function main() {
  ...
  if (missingUsed.length > 0) {
    process.stderr.write(
      ...
      `Put matching .ttf files into ${fontsDir} (or set LINGUA_FONT_DIR) to match editor preview.\n`
    )
  }
