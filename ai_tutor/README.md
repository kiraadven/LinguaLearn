...
AUTO_START_MODEL_SERVICES=false
MODEL_SERVICE_PYTHON_BIN=
...
然后只需要启动主服务（见第 8 步），后端会在启动时自动检测并拉起本机 `FunASR(10095)` 与 `CosyVoice(9880)`。

> 说明：仅当 `FUNASR_WS_URL` / `COSYVOICE_HTTP_URL` 指向本机地址（127.0.0.1/localhost/0.0.0.0）时才会自动拉起；如果你配置的是远程地址，会默认认为是外部服务，不会本地启动。
...
1. 模型服务已启动：`10095` + `9880`（自动拉起或手动拉起都可以）。
2. 主服务已启动：`8080`。
