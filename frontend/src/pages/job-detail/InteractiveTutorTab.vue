<template>
  <div :class="['it-container', { embedded }]">

    <!-- Setup panel (before session starts) -->
    <div v-if="!sessionActive" :class="embedded ? 'it-setup-embedded' : 'it-setup'">
      <div class="it-setup-card">
        <div v-if="!embedded" class="it-logo">🎓</div>
        <h2>互动 AI 老师</h2>
        <p v-if="!embedded" class="it-setup-desc">
    ...
    <!-- Active session -->
    <div v-else :class="['it-session', { 'it-session-embedded': embedded }]">

      <!-- Left sidebar: progress + memory (hidden in embedded mode) -->
      <div v-if="!embedded" class="it-sidebar">
        <!-- Teaching progress -->
      ...
      <!-- Embedded top bar: compact progress + end session button -->
      <div v-if="embedded" class="it-embedded-bar">
        <span class="it-phase-badge" :class="phaseClass">{{ phaseLabel }}</span>
        <span v-if="teachingState.total > 0" style="font-size:12px;color:var(--text3,#888)">
          {{ teachingState.sentence_index >= 0 ? `${teachingState.sentence_index + 1}/${teachingState.total}句` : '等待开始' }}
        </span>
        <button class="it-end-btn" style="margin-left:auto;padding:4px 10px;font-size:12px" @click="endSession">结束</button>
      </div>
      ...
      <div class="it-chat-col">
        ...
        <div class="it-chat-list" ref="chatListEl">
          ...
          <!-- Skip-video hint during intro phase -->
          <div v-if="teachingState.phase === 'intro'" class="it-skip-row">
            <button class="it-skip-btn" @click="skipIntroVideo">
              ▶▶ 我已看完，开始讲解
            </button>
          </div>
        ...
        <div class="it-input-row">
          <!-- Mic button -->
          <button
            :class="['it-mic-btn', { listening: isListening, unavailable: !speechAvailable }]"
            @click="toggleMic"
            :title="isListening ? '停止录音' : (speechAvailable ? '按住说话' : '浏览器不支持语音输入')"
          >{{ isListening ? '🔴' : '🎤' }}</button>

          <input
            v-model="textInput"
            class="it-text-input"
            :placeholder="isListening ? '正在聆听…' : '输入回答，或点击麦克风说话…'"
        ...
        <div class="it-status-bar">
          ...
          <!-- TTS toggle -->
          <button class="it-tts-btn" @click="tutor.toggleTts()" :title="tutor.ttsEnabled.value ? '关闭朗读' : '开启朗读'">
            {{ tutor.ttsEnabled.value ? '🔊' : '🔇' }}
          </button>
          <span v-if="tutor.error.value" class="it-error-text">{{ tutor.error.value }}</span>
        </div>
      </div>

      <!-- Right: current sentence card (hidden in embedded mode) -->
      <div v-if="!embedded" class="it-right-col">
...
const props = defineProps({
  ...
  embedded: { type: Boolean, default: false },  // compact mode: used as side panel in 精听
})
...
function _initRecognition() {
  if (!speechAvailable.value) return null
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // Recognise both Chinese and English (the student may mix languages)
  r.lang = 'zh-CN'
...
function toggleMic() {
  ...
  // Stop Sarah's TTS while the student is speaking
  tutor.stopSpeaking()
...
function skipIntroVideo() {
  // Send a ready signal — the backend detects this and starts sentence teaching
  tutor.sendText('看完了，开始吧')
}
...
.it-mic-btn:hover { border-color: var(--accent, #a78bfa); }
.it-mic-btn.listening {
  border-color: #ef4444;
  background: rgba(239, 68, 68, 0.15);
  animation: pulse-ring 1s infinite;
}
.it-mic-btn.unavailable { opacity: 0.35; cursor: not-allowed; }
...
.it-tts-btn:hover { opacity: 1; background: var(--surface3, #252538); }
...
.it-skip-btn {
  padding: 6px 18px;
  border-radius: 20px;
  border: 1px dashed var(--accent, #a78bfa);
...
/* ── Embedded (side-panel in 精听) mode ── */
.it-container.embedded {
  height: 100%;
  display: flex;
  flex-direction: column;
  min-height: 0;
...
/* Compact setup card when embedded */
.it-setup-embedded {
  flex: 1;
  display: flex;
...
.it-setup-embedded .it-setup-card {
  width: 100%;
  background: none;
  border: none;
  padding: 0;
}
.it-setup-embedded .it-setup-card h2 {
  font-size: 15px;
  margin: 0 0 10px;
}

/* Single-column session layout when embedded */
.it-session-embedded {
...
.it-session-embedded .it-chat-col {
  flex: 1;
  min-height: 0;
  border-radius: 0;
  border: none;
